...
- [ ] **Enkapsulasi Modul (Exports):** Kurangnya properti `exports` pada `ModuleOptions`. Saat ini, semua layanan yang terdaftar di modul mana pun secara efektif bersifat publik setelah di-resolve.
- [ ] **Circular Dependencies:** Terdeteksi tetapi tidak dapat di-resolve. Kurangnya utilitas `forwardRef()` untuk menangani sirkularitas antar-modul atau antar-layanan.
- [ ] **Lifecycle Hooks:** Tidak ada dukungan untuk `OnModuleInit`, `OnApplicationBootstrap`, `OnModuleDestroy`, atau `BeforeApplicationShutdown`.
- [ ] **Async Providers:** Provider di-resolve secara sinkron. Kurang dukungan `useFactory` dengan `async/await`.
- [ ] **Factory Providers:** Belum ada pola provider formal `useFactory`, `useValue`, atau `useExisting`.
...
- [ ] **Token String/Symbol:** DI tampaknya terbatas pada constructor kelas sebagai token. Tidak ada dukungan untuk token berbasis string atau symbol.
...
- [ ] **Interceptors:** Belum ada lapisan Interceptor untuk urusan cross-cutting (transformasi respons, pemetaan stream, dll.).
...
- [ ] **Execution Context:** Pipeline meneruskan `Context` milik Hono. Kurang `ExecutionContext` tingkat framework yang menyediakan metadata tentang kelas dan handler yang sedang dipanggil.
...
- [ ] **Diagnostik:** Meskipun ada beberapa log debug, tidak ada mode "Inspector" untuk pemantauan real-time.
...
### Fase 1: Kekokohan Inti (Jangka Pendek)
1. **Lifecycle Hooks:** Implementasikan `OnModuleInit` dan `OnApplicationBootstrap` untuk memungkinkan layanan melakukan inisialisasi (misalnya, koneksi DB).
2. **Resolusi Circular Dependency:** Perkenalkan `forwardRef()` untuk menangani rantai dependensi yang kompleks.
3. **Module Exports:** Implementasikan enkapsulasi dalam `@Module` untuk membatasi visibilitas layanan.
4. **Provider yang Ditingkatkan:** Tambahkan dukungan untuk `useValue` dan `useFactory` (sinkron).

### Fase 2: Pipeline & DX (Jangka Menengah)
1. **Interceptors:** Tambahkan lapisan Interceptor ke `PipelineExecutor`.
2. **Async Providers:** Aktifkan dukungan `async` untuk `useFactory`.
...
### Fase 3: Ekspansi Ekosistem (Jangka Panjang)
...
2. **Kebocoran Memori:** Verifikasi metode `clear()` di registry bersifat tuntas dan dipanggil saat aplikasi dimatikan.
3. **Penanganan Error di Filter:** Pastikan jika filter melempar error, hal itu tidak merusak seluruh pipeline melainkan jatuh ke global error handler.
